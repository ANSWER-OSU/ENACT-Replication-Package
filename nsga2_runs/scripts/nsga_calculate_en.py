...
RAPL instrumentation
- Reads /sys/class/powercap/intel-rapl/*/energy_uj before and after each
  run_nsga2_for_run() call.
- Handles per-domain counter wraparound via max_energy_range_uj.
- Writes algo_energy_rapl_j into summary.csv (energy consumed by the optimizer
  itself, distinct from the test-suite energy_j objective values).
- Gracefully degrades (NaN) when RAPL is unavailable or unreadable.
...
Outputs
- out/pareto/{run_id}.csv : pareto solutions for that run (includes energy_to_*cov_J)
- out/summary.csv         : one row per run with key pareto points + algo_energy_rapl_j
"""
...
def _rapl_domain_paths(include_dram: bool = True) -> List[str]:
    """
    Return sysfs paths for all readable RAPL package (+ optional dram) domains.

    Typical layout:
      /sys/class/powercap/intel-rapl:0/          <- package-0  (included)
      /sys/class/powercap/intel-rapl:0:0/        <- core       (skipped)
      /sys/class/powercap/intel-rapl:0:1/        <- uncore     (skipped)
      /sys/class/powercap/intel-rapl:0:2/        <- dram       (optional)

    AMD Zen 2+ exposes RAPL via the same powercap tree under the amd_energy
    driver, using identical intel-rapl:* naming, so this function works for
    both vendors without modification.
    """
    paths: List[str] = []
    for base in sorted(glob.glob(f"{_POWERCAP_ROOT}/intel-rapl:*")):
        ...
        if name.startswith("package"):
            paths.append(base)
        elif include_dram and name == "dram":
            paths.append(base)

    return paths


def _read_rapl_uj(domains: List[str]) -> Optional[Dict[str, int]]:
    """
    Read current energy_uj counter for each domain path.

    Returns None if any read fails (no permissions / unsupported hardware).
    """
    snap: Dict[str, int] = {}
    for d in domains:
        ej_path = os.path.join(d, "energy_uj")
        try:
            snap[d] = int(open(ej_path).read().strip())
        except (OSError, ValueError):
            return None
    return snap


def _rapl_max_uj(domain: str) -> int:
    """
    Read max_energy_range_uj for counter-wraparound handling.

    Falls back to 2^32 µJ (~4.3 kJ) when the file is unavailable, which is
    the conservative lower bound used by older kernels.
    """
    try:
        return int(open(os.path.join(domain, "max_energy_range_uj")).read().strip())
    except (OSError, ValueError):
        return 2 ** 32
...
def rapl_delta_joules(
    ...
    """
    Compute total energy in Joules between two RAPL snapshots.

    Handles per-domain counter wraparound using max_energy_range_uj.
    Sums all domains present in 'before'.
    """
    total_uj = 0.0
    for domain, v_before in before.items():
        v_after = after.get(domain)
        if v_after is None:
            continue
        delta = v_after - v_before
        if delta < 0:           # counter wrapped around
            delta += _rapl_max_uj(domain)
        total_uj += delta
    return total_uj / 1_000_000.0   # µJ -> J


def check_rapl_access(include_dram: bool = True) -> Tuple[List[str], bool]:
    """
    Probe RAPL availability at startup.

    Returns (domain_paths, ok).  Prints a warning and returns ([], False) when
    RAPL is unavailable or the process lacks read permission.

    Permission fix (no root required permanently):
      sudo chmod o+r /sys/class/powercap/intel-rapl/*/energy_uj
      sudo chmod o+r /sys/class/powercap/intel-rapl/*/max_energy_range_uj
    """
    domains = _rapl_domain_paths(include_dram=include_dram)
    if not domains:
        warnings.warn(
            "No RAPL domains found under /sys/class/powercap/intel-rapl/. "
            "Energy measurement (algo_energy_rapl_j) will be NaN. "
            "Requires Intel/AMD CPU + Linux kernel >= 3.13."
        )
        return [], False

    snap = _read_rapl_uj(domains)
    if snap is None:
        warnings.warn(
            "RAPL domains exist but are not readable. "
            "Run: sudo chmod o+r "
    ...
    for d in domains:
        try:
            names.append(open(os.path.join(d, "name")).read().strip())
        except OSError:
            names.append(os.path.basename(d))

    print(f"[RAPL] Measuring domains: {', '.join(names)}")
    return domains, True
...
def restrict_to_coverable_lines(
    ...
    Rebuilds a compact bitset matrix with only covered-by-some-test lines.
    """
...
def greedy_reorder_by_marginal_cov_per_joule(
    ...
    Does NOT change total energy/time if all tests in sel run.
    Improves coverage-vs-energy progression (proxy for early termination benefits).
    """
    ...
    while remaining:
        ...
        for t in remaining:
            ...
            if (
                ...
                or (score == best_score and gain == best_gain and (best_energy is None or t_energy < best_energy))
            ):
                best_score = score
                best_gain = gain
                best_t = t
...
def run_nsga2_for_run(
    ...
    - save pareto CSV (includes energy_to_{80,90,95,100}cov_J after Option A reordering)
...
def main():
    """
    Orchestrate:
    - probe RAPL access once at startup
    - load line index (coverage -> tests)
    - load energy per test and align to index tests
    - load PR run table and iterate eligible rows
    - for each run: snapshot RAPL -> run NSGA-II -> snapshot RAPL -> record delta
    - write pareto csv per run and summary.csv with algo_energy_rapl_j column
    """
    ...
    ap.add_argument("--no-dram",            action="store_true",
                    help="Exclude DRAM domain from RAPL measurement")
    args = ap.parse_args()

    os.makedirs(args.out_dir, exist_ok=True)

    # ── RAPL: probe once before any work ──────────────────────────────────────
    rapl_domains, rapl_ok = check_rapl_access(include_dram=not args.no_dram)
    ...
    for _, r in tqdm(df.iterrows(), total=len(df), desc="NSGA-II runs (ordering+cut)"):
        ...
        # ── RAPL: snapshot before ─────────────────────────────────────────────
        snap_before = _read_rapl_uj(rapl_domains) if rapl_ok else None
        ...
        # ── RAPL: snapshot after, compute delta ───────────────────────────────
        if rapl_ok and snap_before is not None:
            snap_after = _read_rapl_uj(rapl_domains)
            s["algo_energy_rapl_j"] = (
                rapl_delta_joules(snap_before, snap_after)
                if snap_after is not None
                else np.nan
            )
        else:
            s["algo_energy_rapl_j"] = np.nan
